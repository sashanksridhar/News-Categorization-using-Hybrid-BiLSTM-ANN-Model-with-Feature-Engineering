...
x_train5 = scale(np.concatenate([encode_sentenceL3(ele, 200) for ele in map(lambda x: x, rowsx1L3)]))
# x_train = np.array([encode_sentence_lstm(ele, 200) for ele in map(lambda x: x, rowsx)])
print(x_train5.shape)

x_train6 = scale(np.concatenate([encode_sentence1L3(ele, 200) for ele in map(lambda x: x, rowsx2L3)]))
# x_train = np.array([encode_sentence_lstm(ele, 200) for ele in map(lambda x: x, rowsx)])
...
visible4 = Input(shape=(200,))

c13 = Dense(256,activation='relu')(visible4)
c23 = Dense(256,activation='relu')(c13)
c33 = Dense(512,activation='relu')(c23)
c43 = Dense(1024,activation='relu')(c33)
c53 = Dense(2048,activation='relu')(c43)
c63 = Dense(1024,activation='relu')(c53)
c73 = Dense(512,activation='relu')(c63)
c83 = Dense(256,activation='relu')(c73)
s13 = Dense(6,activation='relu')(c83)
model5 = Model(inputs=visible4,outputs=s13)

visible5 = Input(shape=(200,))

c133 = Dense(256,activation='relu')(visible5)
c233 = Dense(256,activation='relu')(c133)
c333 = Dense(512,activation='relu')(c233)
c433 = Dense(1024,activation='relu')(c333)
c533 = Dense(2048,activation='relu')(c433)
c633 = Dense(1024,activation='relu')(c533)
c733 = Dense(512,activation='relu')(c633)
c833 = Dense(256,activation='relu')(c733)
s133 = Dense(6,activation='relu')(c833)
model6 = Model(inputs=visible5,outputs=s133)

combined = Concatenate()([model.output, model1.output,model2.output,model3.output,model4.output,model5.output,model6.output])
...
model3 = Model(inputs=[input_tensor,visible,visible1,visible2,visible3,visible4,visible5],outputs=[out1,out2,out3,out4,out5,out6])

model3.compile(optimizer=Adam(lr=1e-3),
               loss='binary_crossentropy',
              metrics=['accuracy'])
x_train = np.array([np.array(token) for token in train_docs])
model3.fit([x_train,x_train1,x_train2,x_train3,x_train4,x_train5,x_train6], [y1,y2,y3,y4,y5,y6], epochs=20, batch_size=128)
model3.save("L123MultiHybridMultioutput20.h5")